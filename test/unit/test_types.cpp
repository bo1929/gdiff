...
TEST_CASE("bin_size computed from bin_shift") {
  params_t<double> p(1, 0.1, 4, 1000, 33.0, 0, 1000, true, false);
  CHECK(p.bin_size == 1);      // 2^0 = 1
  CHECK(p.bin_shift == 0);

  params_t<double> p2(1, 0.1, 4, 1000, 33.0, 3, 1000, true, false);
  CHECK(p2.bin_size == 8);     // 2^3 = 8

  params_t<double> p3(1, 0.1, 4, 1000, 33.0, 10, 1000, true, false);
  CHECK(p3.bin_size == 1024);  // 2^10 = 1024
}

TEST_CASE("tau_bin is ceil(tau / bin_size)") {
  // tau=1000, bin_size=1 -> tau_bin=1000
  params_t<double> p1(1, 0.1, 4, 1000, 33.0, 0, 1000, true, false);
  ...
  params_t<double> p3(1, 0.1, 4, 1001, 33.0, 3, 1000, true, false);
  CHECK(p3.tau_bin == 126);

  // tau=1024, bin_size=1024 -> tau_bin=1
  params_t<double> p4(1, 0.1, 4, 1024, 33.0, 10, 1000, true, false);
  CHECK(p4.tau_bin == 1);
}
...
TEST_CASE("params_t with cm512_t") {
  ...
  params_t<cm512_t> p(8, dths, 4, 5000, 33.0, 2, 1000, true, false);
  CHECK(p.n == 8);
  CHECK(p.bin_size == 4);
