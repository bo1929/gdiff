...
vector<double> prefix_sum(const vector<double> &X) {
  int n = X.size();
...
vector<double> prefix_maxima(const vector<double> &P) {
  int n = P.size();
  vector<double> M(n);
...
vector<double> suffix_min(const vector<double> &P) {
  int n = P.size();
  vector<double> S_min(n + 1, std::numeric_limits<double>::infinity());
  for (int i = n - 1; i >= 0; --i)
    S_min[i] = min(P[i], S_min[i + 1]);
  return S_min;
}

pair<vector<tuple<int, int, int>>, vector<int>>
find_monotone_segments(const vector<double> &P) {
  int n = P.size();
...
build_segment_arrays(const vector<tuple<int, int, int>> &segments,
                     const vector<double> &P) {
  vector<vector<pair<double, int>>> segment_data;
  for (auto &t : segments) {
    int s, e, d;
    std::tie(s, e, d) = t;
    if (d == 1) {
      ...
      pair<double, int> min_val = make_pair(P[s], s);
      for (int i = s; i <= e; ++i)
        if (P[i] < min_val.first)
          min_val = make_pair(P[i], i);
...
// int binary_search_in_segment(const vector<double>& P,
//                              int a,
//                              int K,
//                              int seg_start,
//                              int seg_end,
//                              const vector<pair<double, int>>& seg_vals)
// {
...
int binary_search_in_segment(const vector<double> &P, int a, int K,
  ...
  auto it = std::upper_bound(seg_vals.begin(), seg_vals.end(),
                             make_pair(P[a], std::numeric_limits<int>::max()));
  if (it == seg_vals.begin())
    return -1; // no value <= P[a]
...
vector<pair<int, int>>
monotonic_segment_optimized_intervals(const vector<double> &X, int K) {
  int n = X.size();
  vector<pair<int, int>> intervals;
  vector<double> P = prefix_sum(X);
  vector<double> M = prefix_maxima(P);
  vector<double> S_min = suffix_min(P);
  auto [segments, labels] = find_monotone_segments(P);
  if (segments.size() < 3) {
    return intervals;
  }
  vector<vector<pair<double, int>>> segment_data =
      build_segment_arrays(segments, P);
  vector<int> pm;
  ...
  for (int idx = 0; idx < pm.size(); ++idx) {
    ...
    int b = binary_search_in_segment(P, a, K, seg_start, seg_end,
                                     segment_data[seg_idx]);
    if (b == -1)
      continue;
    if (a > 0 && P[b] <= M[a - 1])
      continue;
    if (b < n && S_min[b + 1] <= P[a])
      continue;
    if (P[a] >= P[b])
...
SBatch::SBatch(sketch_sptr_t sketch, qseq_sptr_t qs, uint32_t hdist_th,
               double dist_th, uint64_t min_length, bool divergent)
    : sketch(sketch), hdist_th(hdist_th), dist_th(dist_th),
      min_length(min_length), divergent(divergent) {
...
void SBatch::seek_sequences(std::ostream &output_stream) {
  ...
  for (bix = 0; bix < batch_size; ++bix) {
    ...
    vector<double> ss = prefix_sum(s);
    vector<double> prefmax(ss.size() + 1);
    prefmax[0] = -std::numeric_limits<double>::max();
    std::inclusive_scan(ss.begin(), ss.end(), prefmax.begin() + 1,
                        [](double a, double b) { return std::max(a, b); });

    vector<double> suffmin(ss.size());
    std::inclusive_scan(ss.rbegin(), ss.rend(), suffmin.rbegin(),
    ...
    for (int a = 1; a < n; ++a) {
      ...
      while (b <= n) {
        if ((prefmax[a - 1] <= ss[b]) && (ss[b] < ss[a]) &&
            (ss[a] <= suffmin[b + 1])) {
          batch_stream << bix << "," << a << "," << b << ',' << n << "\n";
          break;
        }
        b += 1;
      }
    ...
    batch_stream << bix << "," << n << "," << n << ',' << n << "\n";
  }
#pragma omp critical
  output_stream << batch_stream.rdbuf();
}

void SBatch::search_mers(const char *seq, uint64_t len, SSummary &or_summary,
                         SSummary &rc_summary) {
  uint32_t i, l;
  ...
  for (i = l = 0; i < len;) {
    ...
    if (rcenc64_bp < orenc64_bp) {
      ...
      if (sketch->check_partial(orrix)) {
        hdist_or = or_summary.add_matching_mer(sketch, orrix,
                                               lshf->drop_ppos_lr(orenc64_lr));
      }
    } else {
      rcrix = lshf->compute_hash(rcenc64_bp);
      if (sketch->check_partial(rcrix)) {
        hdist_rc = rc_summary.add_matching_mer(
            sketch, rcrix, lshf->drop_ppos_lr(conv_bp64_lr64(rcenc64_bp)));
      }
    }
#else
    orrix = lshf->compute_hash(orenc64_bp);
    ...
    if (hdist_curr <= hdist_th) {
      double sm = (static_cast<double>(hdist_curr) - k * dist_th) /
                  (dist_th * (1 - dist_th));
      if (divergent) {
        s.push_back(-sm);
      } else {
...
uint32_t SSummary::add_matching_mer(sketch_sptr_t sketch, uint32_t rix,
                                    enc_t enc_lr) {
  uint32_t hdist_curr;
...
void SSummary::optimize_likelihood(optimize::HDistHistLLH llhfunc, double rho) {
  ...
  std::pair<double, double> sol_r =
      boost::math::tools::brent_find_minima(llhfunc, 1e-10, 0.5, 16);
  d_llh = sol_r.first;
  v_llh = sol_r.second;
}
