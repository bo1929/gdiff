...
public:
  ...
  Buckets(Buckets&& other) noexcept { swap(other); }
  ...
  void swap(Buckets& other) noexcept;
  void build(uint32_t nrows, vec<uint64_t>&& keys_v);
  void save(std::ostream& os) const;
  void view(const char*& p, const char* end, uint32_t nrows);
  static const char* skip(const char* p, const char* end);
  static uint64_t get_byte_size(uint64_t nkmers, uint64_t nnonempty, uint32_t nrows);
  ...
  bool range(uint32_t bix, const enc_t*& beg, const enc_t*& end) const noexcept
  {
    ...
    const uint32_t rank = brank[bix >> 6] + static_cast<uint32_t>(__builtin_popcountll(word & (bit - 1)));
    beg = enc + start[rank];
  ...
  void prefetch(uint32_t bix) const noexcept
  {
    ...
    __builtin_prefetch(&brank[bix >> 6], 0, 3);
  }

  bool is_nonempty(uint32_t bix) const noexcept
  {
    if (bix >= nrows) return false;
  ...
  [[nodiscard]] bool is_empty() const noexcept { return bitmap == nullptr; }

private:
  // Point the accessors at either the owned vectors or the borrowed buffer.
  void bind() noexcept;
  void set_nrows(uint32_t n) noexcept { nrows = n; }
